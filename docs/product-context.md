...
- 안전 댓글은 Inbox에서 작성자 정보와 원문을 바로 보여준다. 주의 댓글은 작성자 정보와 정리된 의견을 보여주고, 원문은 경고 확인 후 펼친다.
- 위험 댓글은 의미를 훼손하지 않은 순화 요약을 먼저 보여주며, 작성자 정보는 항상 표시하고 원문은 사용자가 경고를 확인한 뒤 펼쳐 볼 수 있다.
...
- 기본 미리보기는 `예시 댓글 · 예상 결과`라고 명시한다. 직접 입력 미리보기는 인증된 서버에서 현재 입력 기준으로 Moderation → Luna → 필요한 경우 Terra와 최종 판정을 실행한다. 입력 댓글과 결과를 DB에 저장하지 않으며, fixture 결과는 별도 표시한다. 영상·답글 문맥이 없는 미리보기는 실제 Inbox 판정과 다를 수 있다.
